...
export class DatadogLambda extends Construct {
  ...
  public addLambdaFunctions(lambdaFunctions: LambdaFunction[], construct?: Construct): void {
    ...
    // defined in DefaultDatadogLambdaProps (if not set by user)
...
export function handleSettingPropDefaults(props: DatadogLambdaProps): DatadogLambdaStrictProps {
  ...
  if (addLayers === undefined) {
    log.debug(`No value provided for addLayers, defaulting to ${DefaultDatadogLambdaProps.addLayers}`);
    addLayers = DefaultDatadogLambdaProps.addLayers;
  }
  if (enableDatadogTracing === undefined) {
    log.debug(
      `No value provided for enableDatadogTracing, defaulting to ${DefaultDatadogLambdaProps.enableDatadogTracing}`,
    );
    enableDatadogTracing = DefaultDatadogLambdaProps.enableDatadogTracing;
  }
  if (enableDatadogASM === undefined) {
    log.debug(`No value provided for enableDatadogASM, defaulting to ${DefaultDatadogLambdaProps.enableDatadogASM}`);
    enableDatadogASM = DefaultDatadogLambdaProps.enableDatadogASM;
  }
  if (enableMergeXrayTraces === undefined) {
    log.debug(
      `No value provided for enableMergeXrayTraces, defaulting to ${DefaultDatadogLambdaProps.enableMergeXrayTraces}`,
    );
    enableMergeXrayTraces = DefaultDatadogLambdaProps.enableMergeXrayTraces;
  }
  if (injectLogContext === undefined) {
    log.debug(`No value provided for injectLogContext, defaulting to ${DefaultDatadogLambdaProps.injectLogContext}`);
    injectLogContext = DefaultDatadogLambdaProps.injectLogContext;
  }
  if (logLevel === undefined) {
    log.debug(`No value provided for logLevel`);
  }
  if (enableDatadogLogs === undefined) {
    log.debug(`No value provided for enableDatadogLogs, defaulting to ${DefaultDatadogLambdaProps.enableDatadogLogs}`);
    enableDatadogLogs = DefaultDatadogLambdaProps.enableDatadogLogs;
  }
  if (captureLambdaPayload === undefined) {
    log.debug(
      `No value provided for captureLambdaPayload, default to ${DefaultDatadogLambdaProps.captureLambdaPayload}`,
    );
    captureLambdaPayload = DefaultDatadogLambdaProps.captureLambdaPayload;
  }
  if (sourceCodeIntegration === undefined) {
    log.debug(
      `No value provided for sourceCodeIntegration, default to ${DefaultDatadogLambdaProps.sourceCodeIntegration}`,
    );
    sourceCodeIntegration = DefaultDatadogLambdaProps.sourceCodeIntegration;
  }

  if (redirectHandler === undefined) {
    log.debug(`No value provided for redirectHandler, default to ${DefaultDatadogLambdaProps.redirectHandler}`);
    redirectHandler = DefaultDatadogLambdaProps.redirectHandler;
  }

  if (grantSecretReadAccess === undefined) {
    log.debug(
      `No value provided for grantSecretReadAccess, default to ${DefaultDatadogLambdaProps.grantSecretReadAccess}`,
    );
    grantSecretReadAccess = DefaultDatadogLambdaProps.grantSecretReadAccess;
  }
